...
- The right side of `Jak firma zarabia` is now Yahoo-backed: use quarterly `fundamentalsTimeSeries` financial statement lines (`total_revenue`, `cost_of_revenue`, `operating_income`, `net_income`) to derive the report’s profit-conversion cards and Sankey cost/profit buckets. Do not scrape TradingView’s `Revenue to profit conversion` widget unless we intentionally add it as a fallback later.
...
- Sankey semantics are intentionally split by provider:
  - left side (`segmenty biznesowe`) comes from cached TradingView source mix and should stay latest-available until TradingView period labels are trustworthy,
  - right side (`koszt dostarczenia`, `koszty operacyjne`, `podatki i pozostale`, `zysk netto`) comes from Yahoo quarterly financials and should stay derived server-side rather than hardcoded in client UI.
