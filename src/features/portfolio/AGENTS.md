...
- Economic currency exposure internals are split into explicit step helpers inside `get-economic-currency-exposure.ts` (raw-holdings lookup, coverage-gap detection, prompt payload/log payload builders, model request wrapper) so the main function remains guard-clause driven without changing response contracts.
...
- Single-portfolio page may render an import progress banner keyed by `?importRun=` plus `?importProvider=` (with legacy `?xtbImportRun=` fallback during transition); the banner polls async broker run state, triggers only the target `PORTFOLIO` snapshot rebuild on completion, and should disappear automatically after success.
...
- Dividend inbox item assembly/finalization is extracted to `src/features/portfolio/server/dividends/dividend-inbox-builders.ts` (past/upcoming builders + booking/currency gating + sorting), while `get-dividend-inbox.ts` stays focused on orchestration/data-fetch.
...
- User portfolio list reads are now also request-memoized via React `cache()` inside `server/get-user-portfolios-private-cached.ts`, so app shell and page-level consumers do not repeat the same auth/portfolio lookup within one render pass.
- Portfolio dashboard deferred loaders share a request-memoized authenticated context in `PortfolioDashboardSection`, so core/recent/dividend/allocation branches do not each repeat `cookies() + auth.getUser()` during the same request.
