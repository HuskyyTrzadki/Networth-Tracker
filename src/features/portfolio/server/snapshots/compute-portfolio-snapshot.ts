...
export async function computePortfolioSnapshot(
  ...
  // Backend: read daily external cashflows and implicit value transfers for TWR.
  // External cashflows = real DEPOSIT/WITHDRAWAL on cash instruments.
  // Implicit transfers = asset legs without any cash leg in the same group.
  ...
  const flowTotalsByCurrency = SNAPSHOT_CURRENCIES.reduce(
    (acc, currency) => {
      // If FX is missing for any flow currency, we null the flow and mark snapshot as partial.
