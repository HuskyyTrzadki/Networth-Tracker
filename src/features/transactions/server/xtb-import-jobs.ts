...
import type {
  ...
  XtbImportRunRowIssue,
  XtbImportRunSummary,
} from "@/features/transactions/lib/xtb-import-types";
...
} from "./xtb-import/shared";
import { sortXtbImportRows } from "./xtb-import/sort-xtb-import-rows";

type SupabaseServerClient = SupabaseClient;
type ImportRunRow = Tables<"xtb_import_runs">;
type ImportRunItemRow = Tables<"xtb_import_run_rows">;
...
const logImportEvent = (
  ...
  console.info(`[xtb-import][job] ${event}`, details);
};
...
const mapBlockingRow = (row: ImportRunItemRow): XtbImportRunRowIssue => ({
  id: row.id,
  rowIndex: row.row_index,
  sourceFileName: row.source_file_name,
  xtbRowId: row.xtb_row_id,
  sourceType: row.source_type,
  tradeDate: row.trade_date,
  status: row.status as XtbImportRunRowIssue["status"],
...
const buildRunSummary = async (
  ...
  const { data: blockingRows, error } = await supabase
    .from("xtb_import_run_rows")
    .select(
      "id,row_index,source_file_name,xtb_row_id,source_type,trade_date,status,error_message"
    )
    .eq("run_id", run.id)
  ...
  return {
    ...
    status: run.status as XtbImportRunSummary["status"],
...
const parseStoredRowPayload = (row: ImportRunItemRow) => {
  const parsed = xtbImportReadyRowSchema.safeParse(row.row_payload);
  if (!parsed.success) {
    throw new Error(
      `${row.source_type} • ${row.source_file_name} • wiersz ${row.xtb_row_id}: zapisany payload importu jest nieprawidłowy.`
    );
  }
...
const toImportRequest = (portfolioId: string, row: XtbImportReadyRow) => {
...
export async function createXtbImportJob(
  ...
  userId: string,
  portfolioId: string,
  rows: readonly XtbImportReadyRow[]
): Promise<CreateXtbImportJobResponse> {
  const startedAt = performance.now();
  const parsedRows = sortXtbImportRows(
    rows.map((row) => xtbImportReadyRowSchema.parse(row))
  );
  const { data: runData, error: runError } = await supabaseAdmin
    ...
    .insert({
      ...
      source_summary: buildSourceSummary(parsedRows),
      updated_at: nowIso(),
    })
    .select("*")
    .single();
  ...
  if (runError || !runData) {
    throw new Error(runError?.message ?? "Nie udało się utworzyć importu XTB.");
  }

  const rowInserts: TablesInsert<"xtb_import_run_rows">[] = parsedRows.map((row, index) => ({
    run_id: runData.id,
    user_id: userId,
    row_index: index,
    source_file_name: row.sourceFileName,
    xtb_row_id: row.xtbRowId,
...
export async function getXtbImportJob(
  supabase: SupabaseServerClient,
  userId: string,
  runId: string
): Promise<XtbImportRunSummary | null> {
...
export async function runXtbImportJob(
  ...
  for (const pendingRow of (pendingRows ?? []) as ImportRunItemRow[]) {
    ...
    try {
      const { request, guardMode, settlementOverride } = toImportRequest(run.portfolio_id, row);
      ...
      const message = resolveUnknownImportErrorMessage(
        caughtError,
        `Nie udało się zapisać wiersza ${pendingRow.xtb_row_id} z importu XTB.`
      );
      const rowDebugLabel = buildXtbImportRowDebugLabel(row);
  ...
  if (updatedRunError || !updatedRunData) {
    throw new Error(updatedRunError?.message ?? "Nie udało się zaktualizować importu XTB.");
  }
