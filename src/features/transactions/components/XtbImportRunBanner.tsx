...
import { useXtbImportRun } from "./use-xtb-import-run";

export function XtbImportRunBanner({
  portfolioId,
}: Readonly<{
  ...
  const runId = searchParams.get("xtbImportRun");
  const { run, isLoading, errorMessage } = useXtbImportRun(runId);
  ...
  if (isLoading || !run) {
    return (
      <Alert className="border-border/70 bg-card/95">
        ...
        <AlertTitle>Import XTB trwa</AlertTitle>
  ...
  if (run.status === "completed") {
    return (
      <Alert className="border-emerald-500/30 bg-emerald-50/70 text-emerald-950">
        ...
        <AlertTitle>Import XTB zapisany</AlertTitle>
        <AlertDescription>
  ...
  if (run.status === "blocked" || run.status === "failed") {
    return (
      <Alert className="border-destructive/30 bg-destructive/5">
        ...
        <AlertTitle>Import XTB zatrzymał się</AlertTitle>
