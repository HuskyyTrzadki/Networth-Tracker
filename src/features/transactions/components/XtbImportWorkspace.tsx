...
import { createXtbImportJob } from "../client/create-xtb-import-job";
import { previewXtbImport } from "../client/preview-xtb-import";
import { revalueXtbPreview } from "../client/revalue-xtb-preview";
import { runXtbImportJob } from "../client/run-xtb-import-job";
import { searchInstruments } from "../client/search-instruments";
import type { XtbImportPreviewResponse, XtbImportPreviewRow } from "../lib/xtb-import-types";
import { XtbImportHoldingsGrid } from "./XtbImportHoldingsGrid";
import { XtbImportReviewTable } from "./XtbImportReviewTable";
import { XtbImportUploadStep } from "./XtbImportUploadStep";
...
export function XtbImportWorkspace({
  portfolios,
  initialPortfolioId,
  ...
  onCompleted?: (result: { portfolioId: string; runId: string }) => void;
  showHeader?: boolean;
}>) {
  const router = useRouter();
  const [selectedPortfolioId, setSelectedPortfolioId] = useState(initialPortfolioId);
  const [files, setFiles] = useState<File[]>([]);
  const [preview, setPreview] = useState<XtbImportPreviewResponse | null>(null);
  const [rows, setRows] = useState<XtbImportPreviewRow[]>([]);
  ...
  const readyRows = useMemo(
    () =>
      rows.filter(
        (row): row is XtbImportPreviewRow & { status: "READY" } => row.status === "READY"
      ),
    [rows]
  );
  ...
  const handleRowsChange = (nextRows: XtbImportPreviewRow[]) => {
    setRows(nextRows);
    ...
    void revalueXtbPreview(nextRows, preview.valuation.baseCurrency)
      .then((nextValuation) => {
  ...
  const handlePreview = async () => {
    ...
    try {
      const nextPreview = await previewXtbImport(
        files,
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Nie udało się przygotować podglądu importu XTB."
      );
    } finally {
  ...
  const handleImport = async () => {
    ...
    const result = await createXtbImportJob(selectedPortfolioId, readyRows).catch(
      (error: unknown) => {
        return {
          errorMessage:
            error instanceof Error
              ? error.message
              : "Nie udało się uruchomić importu XTB.",
        } as const;
      }
    );
    ...
    void runXtbImportJob(result.run.id).catch(() => undefined);
    setIsImporting(false);

    dispatchAppToast({
      title: "Import XTB uruchomiony.",
      description: "Zapisujemy historię w tle. Portfel otworzy się od razu.",
      tone: "success",
    });
    ...
    router.push(`/portfolio/${selectedPortfolioId}?xtbImportRun=${result.run.id}`);
  };
    ...
    <div className="space-y-6">
      {showHeader ? (
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            <h3 className="text-xl font-semibold tracking-tight">Import historii z XTB</h3>
            <p className="text-sm leading-6 text-muted-foreground">
              Importer czyta rozpakowane pliki Excel z sekcji <span className="font-medium text-foreground">Cash Operations</span>, pokazuje podgląd, a potem zapisuje transakcje przez ten sam model co ręczne dodawanie.
            </p>
          </div>
        </div>
      ) : null}
      ...
      {errorMessage ? (
        <Alert className="border-destructive/30 bg-destructive/5">
          <AlertTitle>Import XTB wymaga uwagi</AlertTitle>
            ...
            <div className="rounded-xl border border-border/70 bg-muted/10 px-4 py-3">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {hasCompletePreviewValuation
                  ? "Porównaj tę bieżącą wartość z aktualną wartością portfela w XTB."
                  : hasPreviewValuation
