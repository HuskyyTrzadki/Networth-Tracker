...
- The `Pełna analityka i historia` card now keeps manual entry as the primary full-width action, exposes broker CTAs separately, and currently keeps only `Importuj z XTB` active while `IBKR wkrótce` stays visibly disabled until that provider adapter is ready.
- The broker import dialog now sits on top of the shared broker-import architecture: XTB is the first real provider, it keeps compact export instructions (`Moje transakcje -> Historia konta -> Eksport (Nowy)`), expects unpacked ZIP contents, and embeds the same generic importer shell used by `/transactions/import` against the newly created portfolio.
...
- `BrokerInstructionHero` fallback state should stay on the same warm paper/token system as the rest of onboarding; avoid off-brand dark gradients when an image is missing.
...
- Embedded broker import commit now starts an async import run and navigates to the created portfolio immediately with progress handled there; onboarding must not block on full import replay or snapshot rebuild completion.
