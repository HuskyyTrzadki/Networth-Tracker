    ...
    <section className="space-y-4 border-b border-dashed border-[color:var(--report-rule)] pb-6">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <h3 className="text-3xl font-semibold tracking-tight">Gdzie trafia kazda zlotowka przychodu</h3>
          <StockReportInfoHint
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-2 text-sm md:grid-cols-2">
        {dataset.slices.map((slice) => (
          <div key={slice.label} className="flex items-center justify-between">
            ...
            <span className="font-semibold">{slice.valuePercent.toFixed(1)}%</span>
          </div>
        ))}
      </div>

      <article className="rounded-sm border border-dashed border-[color:var(--report-rule)] p-4">
        <h4 className="text-xl font-semibold tracking-tight">Co mowia priorytety wydatkow</h4>
        <p className="mt-2 text-sm leading-7 text-foreground/90">{dataset.insight}</p>
        <h5 className="mt-4 text-lg font-semibold tracking-tight">Wniosek</h5>
...
function YearComparisonSection() {
  return (
    <section className="space-y-4 border-b border-dashed border-[color:var(--report-rule)] pb-6">
      <div className="flex items-center gap-2">
        <h3 className="text-3xl font-semibold tracking-tight">Ten rok vs ostatni rok</h3>
        <StockReportInfoHint
      ...
      <div className="rounded-sm border border-dashed border-[color:var(--report-rule)] p-4">
        <div className="space-y-3">
          {HOW_THEY_MAKE_MONEY.lastYear.metrics.map((item) => (
            <div
              key={item.label}
              className="grid grid-cols-[1fr_auto_auto] items-end gap-3 border-b border-dashed border-[color:var(--report-rule)] pb-2 last:border-b-0 last:pb-0"
            >
              <p className="text-foreground/80">{item.label}</p>
              <p className="text-2xl font-semibold tracking-tight">{item.value}</p>
              <p className={`text-xl font-semibold ${getChangeTone(item.changeDirection)}`}>
                {item.change}
              </p>
            </div>
          ))}
...
function FreeCashFlowAnalysisSection() {
  return (
    <section className="space-y-4 border-b border-dashed border-[color:var(--report-rule)] pb-6">
      <div className="flex items-center gap-2">
        <h3 className="text-3xl font-semibold tracking-tight">Analiza wolnych przeplywow pienieznych</h3>
        <StockReportInfoHint
      ...
      <div className="overflow-hidden rounded-sm border border-dashed border-[color:var(--report-rule)]">
        <div className="flex h-12">
          <div className="flex w-[50.5%] items-center justify-center bg-profit text-sm font-semibold text-white">FCF</div>
          <div className="flex w-[49.5%] items-center justify-center bg-loss text-sm font-semibold text-white">CapEx</div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-5 text-sm">
        <p className="inline-flex items-center gap-2 text-muted-foreground">
          ...
          Wolne przeplywy <span className="font-semibold text-foreground">$24.5B (50.5%)</span>
        </p>
        <p className="inline-flex items-center gap-2 text-muted-foreground">
          <span className="h-4 w-4 rounded-full bg-loss" />
          CapEx <span className="font-semibold text-foreground">-$24.0B (49.5%)</span>
        </p>
      </div>
      ...
      <article className="space-y-4 rounded-sm border border-dashed border-[color:var(--report-rule)] p-4 text-sm leading-7 text-foreground/90">
        <div>
          <h4 className="text-xl font-semibold tracking-tight">Czym jest wolny przeplyw pieniezny?</h4>
          <p className="mt-2">
        ...
        <div>
          <h4 className="text-xl font-semibold tracking-tight">Czy FCF jest wyzszy czy nizszy od zysku?</h4>
          <p className="mt-2">
            W tym ukladzie FCF (<span className="font-semibold">$24.5B</span>) jest nizszy od zysku netto,
