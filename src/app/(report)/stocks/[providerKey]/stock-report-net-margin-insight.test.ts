import { describe, expect, it } from "vitest";

import type { FundamentalSeriesEvent } from "@/features/stocks/server/types";

import { buildNetMarginInsightWidget } from "./stock-report-net-margin-insight";
import { resolveInsightWidgetCardStat } from "./stock-insights-widget-view";

const createEvent = (
  periodEndDate: string,
  value: number,
  periodType: FundamentalSeriesEvent["periodType"] = "FLOW_QUARTERLY"
): FundamentalSeriesEvent => ({
  periodEndDate,
  value,
  periodType,
  source:
    periodType === "FLOW_ANNUAL" ? "annual_financials" : "quarterly_financials",
});

describe("stock-report-net-margin-insight", () => {
  it("builds quarterly and annual net margin from Yahoo revenue and net income", () => {
    const widget = buildNetMarginInsightWidget({
      quarterlyRevenueEvents: [
        createEvent("2024-03-31", 100),
        createEvent("2024-06-30", 120),
        createEvent("2024-09-30", 110),
        createEvent("2024-12-31", 130),
      ],
      quarterlyNetIncomeEvents: [
        createEvent("2024-03-31", 10),
        createEvent("2024-06-30", 18),
        createEvent("2024-09-30", 11),
        createEvent("2024-12-31", 13),
      ],
      annualRevenueEvents: [createEvent("2023-12-31", 400, "FLOW_ANNUAL")],
      annualNetIncomeEvents: [createEvent("2023-12-31", 40, "FLOW_ANNUAL")],
    });

    expect(widget?.datasets.find((dataset) => dataset.frequency === "quarterly")?.points)
      .toEqual([
        {
          period: "Q1 24",
          primary: 0.1,
          date: "2024-03-31",
        },
        {
          period: "Q2 24",
          primary: 0.15,
          date: "2024-06-30",
        },
        {
          period: "Q3 24",
          primary: 0.1,
          date: "2024-09-30",
        },
        {
          period: "Q4 24",
          primary: 0.1,
          date: "2024-12-31",
        },
      ]);
    expect(widget?.datasets.find((dataset) => dataset.frequency === "annual")?.points).toEqual([
      {
        period: "FY 23",
        primary: 0.1,
        date: "2023-12-31",
      },
      {
        period: "FY 24",
        primary: 0.11304347826086956,
        date: "2024-12-31",
      },
    ]);
  });

  it("formats the widget card stat as percent plus percentage-point delta", () => {
    const widget = buildNetMarginInsightWidget({
      quarterlyRevenueEvents: [
        createEvent("2024-03-31", 100),
        createEvent("2024-06-30", 100),
      ],
      quarterlyNetIncomeEvents: [
        createEvent("2024-03-31", 10),
        createEvent("2024-06-30", 16),
      ],
      annualRevenueEvents: [],
      annualNetIncomeEvents: [],
    });

    const points =
      widget?.datasets.find((dataset) => dataset.frequency === "quarterly")?.points ?? [];

    expect(resolveInsightWidgetCardStat(widget!, points)).toBe("16% • +6 p.p.");
  });
});
